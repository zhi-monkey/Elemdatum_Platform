...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<DataResponseBody> feignException(FeignException e) {
        if ("密码错误！".equals(e.getResponseBody().getMsg())) {
            return buildResponseEntity(HttpStatus.OK,
                    DataResponseFactory.failed(e.getResponseBody().getMsg()));
        }
        if ("用户服务不可用！".equals(e.getResponseBody().getMsg())) {
            return buildResponseEntity(HttpStatus.OK,
                    DataResponseFactory.failed(e.getResponseBody().getMsg()));
        }
        if ("用户信息不存在！".equals(e.getResponseBody().getMsg())) {
            return buildResponseEntity(HttpStatus.OK,
                    DataResponseFactory.failed(e.getResponseBody().getMsg()));
        }
        if ("帐号已锁定！".equals(e.getResponseBody().getMsg())) {
            return buildResponseEntity(HttpStatus.OK,
                    DataResponseFactory.failed(e.getResponseBody().getMsg()));
        }
        // 打印堆栈信息
