...
//    @PreAuthorize(Permissions.ROLE)
    ...
    public DataResponseBody create(@Valid @RequestBody RoleCreateDTO resources) {
        ...
        authCodeCreateDTO.setAuthCode(resources.getName());
        RoleDTO role = roleService.create(resources);
        Long authId = authCodeService.create(authCodeCreateDTO);
        RoleAuthUpdateDTO roleAuthUpdateDTO = new RoleAuthUpdateDTO();
        roleAuthUpdateDTO.setRoleId(role.getId());
        Set<Long> longSet = new HashSet<>();
        longSet.add(authId);
        roleAuthUpdateDTO.setAuthIds(longSet);
        authCodeService.updateRoleAuth(roleAuthUpdateDTO);
        return new DataResponseBody(role);
    }
    ...
    public DataResponseBody update(@Valid @RequestBody RoleUpdateDTO resources) {
        AuthCodeUpdateDTO authCodeUpdateDTO = new AuthCodeUpdateDTO();
        authCodeUpdateDTO.setAuthCode(resources.getName());
        Long authId = authCodeService.getAuthByRole(resources.getId());
        authCodeUpdateDTO.setId(authId);
    ...
    public DataResponseBody updateAuth(@Valid @RequestBody RoleUpdateDTO resources) {
        AuthCodeUpdateDTO authCodeUpdateDTO = new AuthCodeUpdateDTO();
        authCodeUpdateDTO.setAuthCode(resources.getName());
        Long authId = authCodeService.getAuthByRole(resources.getId());
        authCodeUpdateDTO.setId(authId);
        ...
        for (Permission permission : resources.getPermissions()) {
            longSet.add(permission.getId());;
        }
        authCodeUpdateDTO.setPermissions(longSet);
        if (Objects.isNull(authCodeUpdateDTO)) {
            throw new BadCredentialsException("请选择角色信息");
        }
        authCodeService.update(authCodeUpdateDTO);
        return new DataResponseBody();
    ...
    public DataResponseBody delete(@RequestBody RoleDeleteDTO deleteDTO) {
        AuthCodeDeleteDTO authCodeDeleteDTO = new AuthCodeDeleteDTO();
        Long authId = authCodeService.getAuthByRole(new ArrayList<>(deleteDTO.getIds()).get(0));
        Set<Long> longSet = new HashSet<>();
        longSet.add(authId);
        authCodeDeleteDTO.setIds(longSet);
