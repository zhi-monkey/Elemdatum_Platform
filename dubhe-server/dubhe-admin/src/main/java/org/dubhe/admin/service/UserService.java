...
public interface UserService extends AdminUserService, IService<User> {
    ...
    /**
     * 获取角色的用户数量以及总人数，不需要传参
     *
     * @return Map<String, Object> 包含角色的用户数量和总人数
     */
    Map<String, Object> getRoleUserCountsAndTotalCount();
    /**
     * 根据ID获取用户信息
     *
     * @param id 用户Id
     ...
     * 统计用户状态信息
     *
     * @return org.dubhe.admin.domain.vo.UserStatisticsDTO 用户状态信息 VO
     */
    UserStatisticsDTO getUserNum();

    // 查询拥有单一权限的用户
    List<UserDTO> getUsersWithPermission(String permissionName);

    // 查询拥有单一权限的用户（不排除当前用户）
    List<UserDTO> getUsersWithPermissionDoNotExcludeCurrentUser(String permissionName);

    // 查询在线用户数量
    Long getOnlineUserStats();


    // 查询已启用用户数量
    Long getEnabledUserStats();

    String addUserResource(AddUserResourceDTO addUserResourceDTO);

    String releaseUserResource(AddUserResourceDTO addUserResourceDTO);

    UserResourceDTO getUserResourceUsed(Long userId);

    void unbindUsersFromDepartment(Set<Long> ids);

    List<Long> findIdsByUsernameLike(String username);

    User register(@Valid SimpleUserRegisterDTO userRegisterDTO);
}
