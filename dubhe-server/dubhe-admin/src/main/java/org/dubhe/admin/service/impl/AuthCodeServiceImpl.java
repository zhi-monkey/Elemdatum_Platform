...
public class AuthCodeServiceImpl extends ServiceImpl<AuthCodeMapper, Auth> implements AuthCodeService {
    ...
    @Override
    public Long create(AuthCodeCreateDTO authCodeCreateDTO) {
        //获取当前用户id
        ...
        if (CollUtil.isNotEmpty(authCodeCreateDTO.getPermissions())) {
    ...
    public Long getAuthByRole(long roleId) {
        List<Auth> authList = authCodeMapper.selectByRoleId(roleId);
        Long AuthId = authList.get(0).getId();
        return AuthId;
    }
