...
public class MenuServiceImpl implements MenuService {
    ...
    public MenuDTO create(MenuCreateDTO resources) {
        ...
        if (StringUtils.isNotBlank(resources.getPath())) {
            if (menuMapper.findByPath(resources.getPath()) != null ) {
    ...
    public void update(MenuUpdateDTO resources) {
        ...
        if (StringUtils.isNotBlank(resources.getPath())) {
            Menu dbMenu = menuMapper.findByPath(resources.getPath());
            if (dbMenu != null && !dbMenu.getId().equals(menu.getId())) {
                throw new BusinessException("路由地址已存在");
            }
        }
...
//                            }
//                        }
//                        menuVo.setMeta(new MenuMetaVo(menuDTO.getName(), menuDTO.getIcon(), menuDTO.getLayout(), menuDTO.getBackTo(), menuDTO.getHidden(), !menuDTO.getCache()));
        ...
        menuDtos.forEach(menuDTO -> {
            if (menuDTO != null) {
                ...
                if (menuDtoList != null && !menuDtoList.isEmpty()) {
                    menuVo.setChildren(buildMenus(menuDtoList));
                    // 处理是一级菜单并且没有子菜单的情况
                } else if (menuDTO.getPid() == 0 && menuDTO.getType() == 0) {
                    MenuVo menuVo1 = new MenuVo();
                            menuVo1.setMeta(menuVo.getMeta());
                            // 非外链
                            if (MenuTypeEnum.LINK_TYPE.getValue().compareTo(menuDTO.getType()) != 0) {
                    ...
                    // 如果是二级菜单但没有父级目录，将其视为一级目录
                    menuVo.setPath("/" + menuDTO.getPath()); // 设置为顶级路径
                    menuVo.setComponent(StrUtil.isEmpty(menuDTO.getComponent()) ? "Layout" : menuDTO.getComponent());
                }
