...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Autowired
    private DubheDataClient dubheDataClient;

    @Override
    public Map<String, Object> getRoleUserCountsAndTotalCount() {
        Map<String, Object> result = new HashMap<>();

        // 获取各角色的人数统计
        List<RoleUserCountDTO> roleUserCounts = userMapper.getRoleUserCounts();
        result.put("roleUserCounts", roleUserCounts);
    ...
    @Override
    public Long getEnabledUserStats() {
        // 调用 UserMapper 查询已启用的用户数量
    ...
    public Object queryAll(UserQueryDTO criteria, Page page) {
        QueryWrapper<User> wrapper = WrapperHelp.getWrapper(criteria);
        // 排除admin用户
        wrapper.ne("username", User.ADMIN_USERNAME);

        IPage<User> users;
        if (criteria.getRoleId() == null) {
            users = userMapper.selectCollPage(page, wrapper);
        } else {
            users = userMapper.selectCollPageByRoleId(page, wrapper, criteria.getRoleId());
        }
        ...
        return PageUtil.toPage(users, userDTOs);
    }
    ...
    public List<UserDTO> queryAll(UserQueryDTO criteria) {
        ...
        QueryWrapper<User> wrapper = WrapperHelp.getWrapper(criteria);
        wrapper.eq("create_user_id", curUserId);
        List<User> users = userMapper.selectCollList(wrapper);
    ...
    public User register(SimpleUserRegisterDTO userRegisterDTO) {
        PasswordEncoder passwordEncoder = PasswordEncoderFactory.getPasswordEncoder();
        if (!Objects.isNull(userMapper.findByUsername(userRegisterDTO.getUsername()))) {
            throw new BusinessException("用户名已存在");
        }
        if (userMapper.findByEmail(userRegisterDTO.getEmail()) != null) {
            throw new BusinessException("邮箱已存在");
        }
        Role visitorRole = roleMapper.findVisitorRole();
        if (Objects.isNull(visitorRole)) {
            throw new BusinessException("请先联系管理员创建游客角色");
        }
        ...
        user.setCpuUsed(0L);
        user.setMemoryUsed(0L);
        user.setGpuMemoryUsed(0L);
        user.setVGpuCoresUsed(0L);
        // 不填默认启用
        user.setEnabled(!Objects.isNull(userRegisterDTO.getEnabled()) ? userRegisterDTO.getEnabled() : true);

        userMapper.insert(user);
        roleMapper.tiedUserRole(user.getId(), visitorRole.getId());

        // 通知所有管理员有新用户注册
        notifyAdminsNewUserRegistered(user);
     ...
     * 通知管理员有新用户注册
     */
    private void notifyAdminsNewUserRegistered(User user) {
        try {
            // 查询角色为"管理员"或"管理人员"的用户
            List<User> admins = userMapper.findAdminUsers();
            if (admins == null || admins.isEmpty()) {
                log.warn("未找到管理员或管理人员，无法发送新用户注册通知");
                return;
            }
            ...
            for (User admin : admins) {
                try {
                    dubheDataClient.createNotification(
                            admin.getId(),
                            1, // INFO 类型
                            "MSG_USER_REGISTERED",
                    ...
                    log.error("发送新用户注册通知给管理员[{}]失败: {}", admin.getUsername(), e.getMessage());
                }
            }
        } catch (Exception e) {
            log.error("通知管理员新用户注册异常: {}", e.getMessage());
        }
    }
    ...
    public UserDTO create(UserCreateDTO resources) {
        ...
        userMapper.updateDepartmentId(user.getId(), resources.getDepartmentId());
        return userConvert.toDto(user);
    }
    ...
    public void delete(Set<Long> ids) {
        if (!CollectionUtils.isEmpty(ids)) {
            ...
            ids.forEach(id -> {
                ...
                dubheDataClient.handleUserRemoval(id);
            });
        }
    }
...
//        }
        ...
        if (ResponseCode.SUCCESS.compareTo(restResult.getCode()) == 0 && !Objects.isNull(restResult.getData())) {
            ...
            if (CollUtil.isEmpty(permissions)) {
                throw new BusinessException(BaseErrorCodeEnum.ROLE_PERMISSION_ERROR);
            }
            // 返回 token 与 用户信息
            ...
            // 存储用户到bitmap
            if (!redisUtils.getBit(UserConstant.USER_ONLINE, user.getId())) {
                redisUtils.setBit(UserConstant.USER_ONLINE, user.getId(), true);
            }
        }
    ...
    public DataResponseBody logout(String accessToken) {
        ...
        // 将用户从bitmap中清除
        Long curUserId = userContextService.getCurUserId();
        if (redisUtils.getBit(UserConstant.USER_ONLINE, curUserId)) {
            redisUtils.setBit(UserConstant.USER_ONLINE, curUserId, false);
        }
//        return authServiceClient.logout(accessToken);
     ...
     */
    @Override
    public UserStatisticsDTO getUserNum() {
        Integer userOnNum = userMapper.selectCount(new LambdaQueryWrapper<User>()
                .eq(User::getDeleted, SwitchEnum.getBooleanValue(SwitchEnum.OFF.getValue()))
                .eq(User::getEnabled, SwitchEnum.getBooleanValue(SwitchEnum.ON.getValue())));
        Integer userNum = userMapper.selectCount(new LambdaQueryWrapper<User>()
                .eq(User::getDeleted, SwitchEnum.getBooleanValue(SwitchEnum.OFF.getValue())));
        Integer userOffNum = userNum - userOnNum;

        return UserStatisticsDTO.builder().userNum(userNum).userOffNum(userOffNum).userOnNum(userOnNum).build();
    }
    ...
    public List<UserDTO> getUsersWithPermission(String permissionName) {
        List<User> usersWithPermission = userMapper.findUsersWithPermission(permissionName);
        Long curUserId = userContextService.getCurUserId();
        List<User> collect = usersWithPermission.stream().filter((User user) -> !Objects.equals(user.getId(), curUserId)).collect(Collectors.toList());
        return userConvert.toDto(collect);
    }

    @Override
    public List<UserDTO> getUsersWithPermissionDoNotExcludeCurrentUser(String permissionName) {
        List<User> usersWithPermission = userMapper.findUsersWithPermission(permissionName);
        return userConvert.toDto(usersWithPermission);
    }

//    @Override
//    public Long getOnlineUserStats() {
//        return redisUtils.getBitCount(UserConstant.USER_ONLINE);
//    }

    @Override
    public Long getOnlineUserStats() {
        Long onlineCount = redisUtils.getBitCount(UserConstant.USER_ONLINE);
        Long totalUserCount = Long.valueOf(userMapper.getTotalUserCount());

        if (onlineCount != null && totalUserCount != null) {
            return onlineCount > totalUserCount ? totalUserCount : onlineCount;
        } else if (onlineCount != null) {
            return onlineCount;
        } else if (totalUserCount != null) {
            return totalUserCount;
        } else {
            return 0L; // 或根据业务需求返回其他默认值
        }
    }

    @Override
    public String addUserResource(AddUserResourceDTO addUserResourceDTO) {
        Long cupUseNeed = addUserResourceDTO.getCpuUse();
        Long memoryUseNeed = addUserResourceDTO.getMemoryUse();
        Long gpuMemoryUseNeed = addUserResourceDTO.getGpuMemoryUse();
        Long vGpuCoresUseNeed = addUserResourceDTO.getVGpuCoresUse();
        User user = userMapper.selectById(addUserResourceDTO.getUserId());
        if (user == null) {
            throw new BusinessException("用户不存在");
        }
        // 获取用户当前所在的department
        Department department = departmentMapper.selectById(user.getDepartmentId());
        if (user.getCpuUsed() + cupUseNeed > department.getCpuLimit()) {
            return User.CPU_USED_OVER_LIMIT;
        }
        if (user.getMemoryUsed() + memoryUseNeed > department.getMemoryLimit()) {
            return User.MEMORY_USED_OVER_LIMIT;
        }
        if (user.getGpuMemoryUsed() + gpuMemoryUseNeed > department.getGpuMemoryLimit()) {
            return User.GPU_MEMORY_USED_OVER_LIMIT;
        }
        if (user.getVGpuCoresUsed() + vGpuCoresUseNeed > department.getVGpuCoresLimit()) {
            return User.V_GPU_CORES_USED_OVER_LIMIT;
        }
        int i = userMapper.updateResourceUsed(addUserResourceDTO.getUserId(), cupUseNeed, memoryUseNeed, gpuMemoryUseNeed, vGpuCoresUseNeed);
        //log.info("增加用户{}的{}个CPU和{}GB内存{}GB显存{}%算力{}", user.getUsername(), cupUseNeed, memoryUseNeed, gpuMemoryUseNeed, vGpuCoresUseNeed, i > 0 ? "成功" : "失败");
        return i > 0 ? User.DISTRIBUTED_SUCCESS : User.DISTRIBUTED_FAILED_UNKNOWN_REASON;
    }

    @Override
    public String releaseUserResource(AddUserResourceDTO addUserResourceDTO) {
        Long cupUseNeed = addUserResourceDTO.getCpuUse();
        Long memoryUseNeed = addUserResourceDTO.getMemoryUse();
        Long gpuMemoryUseNeed = addUserResourceDTO.getGpuMemoryUse();
        Long vGpuCoresUseNeed = addUserResourceDTO.getVGpuCoresUse();
        ...
        if (user.getCpuUsed() < cupUseNeed) {
            return User.RELEASING_CPU_TO_LARGE;
        }
        if (user.getMemoryUsed() < memoryUseNeed) {
            return User.RELEASING_MEMORY_TO_LARGE;
        }
        if (user.getGpuMemoryUsed() < gpuMemoryUseNeed) {
            return User.RELEASING_GPU_MEMORY_TO_LARGE;
        }
        if (user.getVGpuCoresUsed() < vGpuCoresUseNeed) {
            return User.RELEASING_V_GPU_CORES_TO_LARGE;
        }
        int i = userMapper.updateResourceUsed(addUserResourceDTO.getUserId(), -cupUseNeed, -memoryUseNeed, -gpuMemoryUseNeed, -vGpuCoresUseNeed);
        //log.info("释放用户{}的{}个CPU和{}GB内存{}GB显存{}%算力{}", user.getUsername(), cupUseNeed, memoryUseNeed, gpuMemoryUseNeed, vGpuCoresUseNeed, i > 0 ? "成功" : "失败");
        return i > 0 ? User.RELEASED_SUCCESS : User.RELEASED_FAILED_UNKNOWN_REASON;
    }

    @Override
    public UserResourceDTO getUserResourceUsed(Long userId) {
        ...
        List<User> usersInDepartment = userMapper.findUsersInDepartment(department.getId());
        Long cpuUsed = 0L, memoryUsed = 0L, gpuMemoryUsed = 0L, vGpuCoresUsed = 0L;
        for(User u : usersInDepartment) {
            cpuUsed += u.getCpuUsed();
            memoryUsed += u.getMemoryUsed();
            gpuMemoryUsed += u.getGpuMemoryUsed();
            vGpuCoresUsed += u.getVGpuCoresUsed();
        }
        return new UserResourceDTO(userId, cpuUsed, memoryUsed, gpuMemoryUsed, vGpuCoresUsed, department.getCpuLimit(), department.getMemoryLimit(), department.getGpuMemoryLimit(), department.getVGpuCoresLimit());
    }

    @Override
    public void unbindUsersFromDepartment(Set<Long> ids) {
        userMapper.unbindUserDepartmentByDepartmentIds(ids);
    }

    /**
     * 根据用户名模糊匹配返回用户ID列表
     * @param username 用户名模糊匹配字符串
     * @return 匹配的用户ID列表
     */
    @Override
    public List<Long> findIdsByUsernameLike(String username) {
        // 构建查询条件
