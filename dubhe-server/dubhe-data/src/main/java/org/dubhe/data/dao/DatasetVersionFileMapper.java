...
public interface DatasetVersionFileMapper extends BaseMapper<DatasetVersionFile> {
    ...
    List<DatasetVersionFile> findByDatasetIdAndVersionNameWithoutStatus(@Param("datasetId") Long datasetId,
                                                           @Param("versionName") String versionName);
    ...
    // 根据数据集和游标查找文件状态列表
    List<Integer> findFileStatusListByDatasetAndOffset(@Param("datasetId") Long datasetId, @Param("startOffset") Long startOffset,@Param("fileNum") Long fileNum);
    ...
    /**
     * 删除指定数据集和版本下 status 为 0 或 1 的版本文件记录
     */
    void deleteStatus01ByDatasetAndVersion(@Param("datasetId") Long datasetId,
                                           @Param("versionName") String versionName);

    /**
     * 将指定数据集和版本下的 changed 重置为 0
     */
    void resetChangedByDatasetIdAndVersionName(@Param("datasetId") Long datasetId,
                                               @Param("versionName") String versionName);
     ...
     * @param datasetId    数据集ID
     * @param versionName  数据集版本名称
     * @return Integer   数据集文件状态统计数据
     */
    Integer getDatasetVersionAllFileCount(@Param("datasetId") Long datasetId, @Param("versionName") String versionName);
    ...
    /**
     * 批量更新文件状态为未标注, versionName可以缺省为null
     *
     * @param datasetId     数据集ID
     * @return int 更新数量
     */
    int updateFileStatus2UnAnnotated(@Param("datasetId") Long datasetId, @Param("versionName") String versionName);



    /**
     * 根据数据集和偏移量范围统计已标注文件数量
     *
     * @param datasetId     数据集ID
     * @param versionName   版本名称
     * @param startOffset   起始偏移量
     * @param limitNum      查询数量
     * @return Integer      已标注文件数量
     */
    Integer countAnnotatedFilesByOffset(@Param("datasetId") Long datasetId,
                                        @Param("versionName") String versionName,
    ...
    void updateAnnotationStatusByIds(@Param("annotationStatus") Integer annotationStatus,
     ...
     * @param versionName   版本名称
     * @return List<Long>   版本文件id列表
     */
    List<Long> getVersionFileIdsByFileNames(
            @Param("datasetId") Long datasetId,
    ...
    // ===== 修改 DatasetVersionFileMapper 中的查询方法 =====
    ...
    @Select("<script>" +
            ...
            "</foreach>" +
            "</script>")
    List<DatasetVersionFile> getVersionFilesByFileIds(@Param("datasetId") Long datasetId,
                                                      @Param("versionName") String versionName,
                                                      @Param("fileIds") List<Long> fileIds);
    ...
    @Update("<script>" +
            ...
            "UPDATE data_dataset_version_file SET " +
            "annotation_status = #{item.annotationStatus}" +
            "<if test='item.changed != null'>, changed = #{item.changed}</if>" +
            " WHERE id = #{item.id} AND dataset_id = #{item.datasetId}" +
            "</foreach>" +
            "</script>")
    void updateStatusBatch(@Param("list") List<DatasetVersionFile> datasetVersionFiles);


    @Select("<script>" +
            "SELECT COUNT(*) " +
            "FROM data_dataset_version_file " +
            "WHERE dataset_id IN " +
            ...
            " AND status != 1" +
            "</script>")
    Integer getDatasetVersionAllFileCountByIds(@Param("datasetIds") List<Long> datasetIds);

    /**
     * 批量查询多个数据集的 changed 文件计数
     * @param datasetInfos 数据集信息列表（包含 datasetId 和 versionName）
     * @return List<Map> 每个数据集的 changed 文件计数，Map包含 datasetId 和 count 字段
     */
    List<Map<String, Object>> batchCountChangedFiles(@Param("datasetInfos") List<DatasetChangedInfo> datasetInfos);

    /**
     * 数据集 changed 文件信息
     */
    class DatasetChangedInfo {
        private Long datasetId;
        private String versionName;

        public DatasetChangedInfo(Long datasetId, String versionName) {
