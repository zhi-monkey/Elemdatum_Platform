...
public interface LabelMapper extends BaseMapper<Label> {
    ...
    /**
     * 根据标签ID集合查询标签 注意这里的@Param("labelIds")是要和foreach中的collection中对应的, 如果params中不指定则需要使用默认的"list"
     *
     * @param labelIds 标签ID集合
     * @return List<Label> 标签集合
     */
    @Select("<script>" +
            "SELECT * FROM data_label " +
            "WHERE id IN " +
            "<foreach collection='labelIds' item='id' open='(' separator=',' close=')'>" +
            "#{id}" +
            "</foreach>" +
            "</script>")
    List<Label> findLabelByIds(@Param("labelIds") List<Long> labelIds);

    @Select("select dl.id, dl.name, dl.color, dl.type from data_label dl left join data_dataset_label ddl on dl.id = ddl.label_id where ddl.dataset_id = #{datasetId}  and dl.deleted = 0")
    List<Label> listLabelByDatasetIdAndVersionName(Long datasetId, String versionName);
}
