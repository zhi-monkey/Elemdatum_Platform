...
public class EnhanceTaskSplitBO implements Serializable {
    ...
    public EnhanceTaskSplitBO(Long id, List<File> files, Long datasetId, String versionName, Map<Long, DatasetVersionFile> datasetVersionFilesMap, List<Integer> types, FileUtil fileUtil) {
    ...
    public EnhanceTaskSplitBO(Long id, List<File> files, Long datasetId, String versionName,
                              Map<Long, DatasetVersionFile> datasetVersionFilesMap,
                              Integer enhanceType, FileUtil fileUtil) {

        List<Integer> enhanceTypes = new ArrayList<>();
        if (enhanceType != null) {
    ...
    public String createEnhanceFilePath(String suffix, DatasetFileBO datasetFileBO) {
        ...
        String fileFullName = filePath.substring(filePath.lastIndexOf("/") + MagicNumConstant.ONE, filePath.length());
        String fileName = fileFullName.substring(0, fileFullName.lastIndexOf("."));
        String fileSuffix = fileFullName.substring(fileFullName.lastIndexOf("."), fileFullName.length());
        return enhanceFilePath + "/" + fileName + suffix + fileSuffix;
    }
