...
public class AnnotationCompleteState extends AbstractDataState {
    ...
    public void enhanceErrorEvent(Integer primaryKeyId, String dataVersionName) {
        // 数据增强失败
        LogUtil.info(LogEnum.STATE_MACHINE, "数据增强失败，数据集信息：ID:{}, VERSION_NAME:{}", primaryKeyId, dataVersionName );
    ...
    public void zipImportEvent(Integer primaryKeyId) {
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【标注完成】 执行事件前内存中状态机的状态 :{} ", dataStateMachine.getMemoryDataState());
        LogUtil.debug(LogEnum.STATE_MACHINE, " 接受参数： {} ", primaryKeyId);
        datasetMapper.updateStatus(Long.valueOf(primaryKeyId), DataStateEnum.ZIP_IMPORT_STATE.getCode());
        dataStateMachine.setMemoryDataState(dataStateMachine.getZipImportState());
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【标注完成】 执行事件后内存状态机的切换： {}", dataStateMachine.getMemoryDataState());
    }
