...
public class AnnotationNotDistinguishFileState extends AbstractFileState {
    ...
    /**
     * 文件 自动标注未识别-->手动标注文件点击保存-->标注中
     *
     * @param datasetVersionFile 版本文件对象
     */
    @Override
    public void manualAnnotationSaveEvent(DatasetVersionFile datasetVersionFile) {
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【自动标注未识别】 执行事件前内存中状态机的状态 :{} ", fileStateMachine.getMemoryFileState());
    ...
    /**
     * 文件 自动标注未识别-->点击完成-->标注完成
     *
     * @param datasetVersionFile 业务对象
     */
    @Override
