...
/**
 * @description 子任务手动标注中状态类
 * @date 2020-08-27
 */
@Component
public class ManualAnnotationSubtaskState extends AbstractDataTeamSubtaskState {
    ...
    public void finishManualSubtaskEvent(DataTeamSubtask subtask){
        ...
        switch (status){
            ...
            case NOT_ANNOTATION_SUBTASK_STATE:
                //未标注
                dataTeamSubtaskStateMachine.doStateChange(subtask.getId(),DataTeamSubtaskStateEnum.NOT_ANNOTATION_SUBTASK_STATE.getCode(),dataTeamSubtaskStateMachine.getNotAnnotationSubtaskState());
                break;
            default:
    ...
    public void annotationCompleteSubtaskEvent(DataTeamSubtask subtask){
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【未标注】 执行事件前内存中状态机的状态 :{} ", dataTeamSubtaskStateMachine.getMemorySubtaskState());
        LogUtil.debug(LogEnum.STATE_MACHINE, " 接受参数： {} ", subtask);
        dataTeamSubtaskMapper.updateStatus(subtask.getId(), DataTeamSubtaskStateEnum.FINISH_ANNOTATION_SUBTASK_STATE.getCode());
        dataTeamSubtaskStateMachine.setMemorySubtaskState(dataTeamSubtaskStateMachine.getAnnotationCompleteSubtaskState());
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【未标注】 执行事件后内存状态机的切换： {}", dataTeamSubtaskStateMachine.getMemorySubtaskState());
    }
}