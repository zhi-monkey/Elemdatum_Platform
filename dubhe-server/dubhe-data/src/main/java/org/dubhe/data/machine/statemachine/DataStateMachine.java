...
public class DataStateMachine extends AbstractDataState implements Serializable {
     ...
     */
    @Autowired
    private ZipImportState zipImportState;
    ...
    @Getter
    @Autowired
    private final AbstractDataState errorState;
     ...
     *
     * @param primaryKeyId    业务ID
     * @param dataVersionName
     */
    @Override
    public void enhanceErrorEvent(Integer primaryKeyId, String dataVersionName) {
        initMemoryDataState(primaryKeyId);
        memoryDataState = SpringContextHolder.getBean(DataStateEnum.ANNOTATION_COMPLETE_STATE.getStateMachine());
        memoryDataState.enhanceErrorEvent(primaryKeyId, dataVersionName);
    }
    ...
    /**
     * 压缩包导入事件 未标注 --> 导入压缩包 --> 导入中
     *
     * @param primaryKeyId 业务ID
     */
    @Override
    public void zipImportEvent(Integer primaryKeyId) {
        initMemoryDataState(primaryKeyId);
        if (memoryDataState == zipImportState) {
            throw new StateMachineException(ErrorMessageConstant.DATASET_CHANGE_ERR_MESSAGE);
        }
        memoryDataState.zipImportEvent(primaryKeyId);
    }
    ...
    public void zipImportFinishEvent(Dataset dataset) {
        ...
        memoryDataState.zipImportFinishEvent(dataset);
    }

    /**
     * 自动标注完成 自动标注中 --> 标注中/标注完成
     *
     * @param dataset 数据集详情
     */
    @Override
    public void automaticLabelingFinishEvent(Dataset dataset) {
        initMemoryDataState(dataset.getId().intValue());
        if (memoryDataState != automaticLabelingState) {
            throw new StateMachineException(ErrorMessageConstant.DATASET_CHANGE_ERR_MESSAGE);
        }
        memoryDataState.automaticLabelingFinishEvent(dataset);
    }
     ...
     */
    @Override
    public void autoLabelCompleteEvent(Dataset dataset) {
        initMemoryDataState(dataset.getId().intValue());
        if (memoryDataState != automaticLabelingState) {
            throw new StateMachineException(ErrorMessageConstant.DATASET_CHANGE_ERR_MESSAGE);
        }
        memoryDataState.autoLabelCompleteEvent(dataset);
    }
    ...
    public void importErrorEvent( Dataset dataset) {
        ...
        //     throw new StateMachineException(ErrorMessageConstant.DATASET_CHANGE_ERR_MESSAGE);
        // }
        memoryDataState.importErrorEvent(dataset);
    }
}
