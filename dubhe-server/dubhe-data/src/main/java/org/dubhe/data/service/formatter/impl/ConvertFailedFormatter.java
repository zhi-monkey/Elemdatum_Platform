...
@Component
public class ConvertFailedFormatter implements NotificationFormatter {

    @Override
    public NotificationOperationTypeEnum supportType() {
        return NotificationOperationTypeEnum.CONVERT_FAILED;
    }
    ...
    public String format(String payload) {
        try {
            ...
            return String.format("【标准化】转换任务【%s】（子任务%s）执行失败！", generationName, jobName);
        } catch (Exception e) {
            return "转换任务执行失败";
        }
    }
