...
@Component
public class GuidedTrainFailedFormatter implements NotificationFormatter {

    @Override
    public NotificationOperationTypeEnum supportType() {
        return NotificationOperationTypeEnum.GUIDED_TRAIN_FAILED;
    }
    ...
    public String format(String payload) {
        try {
            ...
            return String.format("【引导式】【%s】训练任务执行失败！", generationName);
        } catch (Exception e) {
            return "引导式训练任务执行失败";
        }
    }
