    ...
    public void update(Long labelGroupId, LabelGroupCreateDTO labelGroupCreateDTO) {
        ...
        if (!CollectionUtils.isEmpty(labelList)) {
            ...
            if (!CollectionUtils.isEmpty(dbLabels)) {
                ...
                //校验标签组是否关联数据集
                int count = datasetService.getCountByLabelGroupIdAndDeleted(labelGroupId,false);
...
//        }
        // 校验标签组是否被数据集引用（仅查询未删除的数据集）
        if (datasetService.getCountByLabelGroupIdAndDeleted(labelGroupId, false) > 0) {
            //LogUtil.info(LogEnum.BIZ_DATASET, "数据集未删除数量：",datasetService.getCountByLabelGroupIdAndDeleted(labelGroupId, false));
            throw new BusinessException(ErrorEnum.LABELGROUP_LABEL_GROUP_QUOTE_DEL_ERROR);
        }

        // 校验标签组是否被点云数据集引用
        if (pcDatasetMapper.getCountPCByLabelGroupId(labelGroupId) > 0) {
            //LogUtil.info(LogEnum.POINT_CLOUD, "点云：",pcDatasetMapper.getCountPCByLabelGroupId(labelGroupId));
...
//        // 如果所有引用该标签组的数据集已删除，则软删除标签组
//        if (datasetService.getCountByLabelGroupIdAndDeleted(labelGroupId, false) == 0 &&
//                pcDatasetMapper.getCountPCByLabelGroupId(labelGroupId) == 0) {
            // 删除标签组数据
...
//        }
    ...
    public Map<String, Object> listVO(Page<LabelGroup> page, LabelGroupQueryVO labelGroupQueryVO) {
        ...
        // 根据name进行查询
        // 假设我们进行精确匹配查询，不涉及id
        labelGroupQueryVO.setId(null);  // 确保不通过id进行查询
        return queryLabelGroups(page, labelGroupQueryVO, null);
    }
...
//            queryWrapper.eq("create_user_id", userContextService.getCurUserId());
//        }
    ...
    @Override
    public List<LabelGroup> getAllList() {

        // 使用 QueryWrapper 构建查询条件，筛选未删除的标签组
        QueryWrapper<LabelGroup> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("deleted", MagicNumConstant.ZERO); // 逻辑未删除条件
        // 从数据库中查询并返回结果
