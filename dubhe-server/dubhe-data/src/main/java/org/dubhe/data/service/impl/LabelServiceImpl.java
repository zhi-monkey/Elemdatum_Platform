...
public class LabelServiceImpl extends ServiceImpl<LabelMapper, Label> implements LabelService {
    ...
    @Override
    public List<Long> saveLabelByReadFile(String appZipPath) {
        // appZipPath : cz-prod/package/test_make_bin-1-1742885660506.zip  absArchiveUrl : ${storage.file-store-root(-windows)-path}/cz-prod/package/test_make_bin-1-1742885660506.zip
        String absArchiveUrl = fileUtil.getOriginFileAbsPath(appZipPath);
        // 拿到fileName, 因为算法包不会套一层文件夹, 所以这里需要指定解压到以他文件名命名的文件夹 fileName : test_make_bin-1-1742885660506/
        String parentFolder = org.dubhe.biz.base.utils.StringUtils.substringBeforeLast(Paths.get(absArchiveUrl).getFileName().toString(), ".") + "/";

        List<Long> labelList = new ArrayList<>();
        try {
            List<String> filePaths = minioUtil.decompress(new java.io.File(absArchiveUrl), bucket,"package/" + parentFolder);
            if (filePaths.isEmpty()) {
                return null;
            }
            // 拿到解压后的文件夹路径 ${storage.file-store-root(-windows)-path}/cz-prod/package/test_make_bin-1-1742885660506

            String classesFile = null ;

            boolean isExistClassesFile = false;
            for (String path : filePaths) {
                if (path.contains("classes.txt")) {
                    isExistClassesFile = true;
                    classesFile = fileUtil.getOriginFileAbsPath( bucket + "/" + path);
                }
            }
            if (!isExistClassesFile) {
                throw new BusinessException("标签文件不存在");
            }
            List<String> lines = Files.readAllLines(Paths.get(classesFile));

            lines.forEach(line -> {
                if(StringUtils.isNotBlank(line)) {
                    String categoryName = line.trim();
                    Label label = Label.builder().name(categoryName)
                            .color(ConversionUtil.generateRandomColor())
                            .build();
                    labelMapper.insert(label);
                    // Label的id的IdType是Auto, 这意味着在调用 insert 方法后，MyBatis-Plus 会自动将生成的主键值填充到实体对象中
                    labelList.add(label.getId());
                    
                    // 同时添加到标签库中（如果不存在）
                    labelTemplateService.findOrCreateByName(categoryName);
                }
            });
            
            // 删除MinIO中的文件
            try {
                String prefix = "package/" + parentFolder;
                
                // 使用del方法一次性删除所有对象
                minioUtil.del(bucket, prefix);
                
                // 删除文件夹壳子
                String localPath = fileUtil.getOriginFileAbsPath(bucket + "/package/" + parentFolder);
                java.io.File localFolder = new java.io.File(localPath);
                if (localFolder.exists() && localFolder.isDirectory()) {
                    org.apache.commons.io.FileUtils.deleteDirectory(localFolder);
                    LogUtil.info(LogEnum.BIZ_DATASET, "成功删除本地解压文件夹: {}", localPath);
                }
                
                LogUtil.info(LogEnum.BIZ_DATASET, "成功删除MinIO中的解压文件: {}", prefix);
            } catch (Exception e) {
                LogUtil.error(LogEnum.BIZ_DATASET, "删除解压文件失败: {}", e.getMessage());
                // 仅记录日志，不抛出异常，避免影响主流程
            }
        } catch (Exception e) {
            throw new BusinessException(ErrorEnum.FILE_UNZIP_ERROR.getMsg(), e);
        }
        return labelList;
    }

    @Override
    public List<Label> findLabelByIds(List<Long> labelIds) {
        if (CollectionUtils.isEmpty(labelIds)) {
            return new ArrayList<>();
        }
        return baseMapper.findLabelByIds(labelIds);
    }
