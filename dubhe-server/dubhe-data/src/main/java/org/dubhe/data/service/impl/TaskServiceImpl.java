...
//    }
    ...
    public void finishTaskFile(EnhanceTaskSplitBO enhanceTaskSplitBO, Integer fileNum) {
        ...
        if (task.getFinished() >= task.getTotal()) {
            ...
            datasetOperationEventService.createAndInsertEvent(task.getDatasetId(),new Date(),"数据增强任务完成",DatasetOperationEvent.EventType.INFO,DatasetOperationEvent.OperationType.DATA_AUGMENTATION);
            createEnhanceFinishedNotification(task.getDatasetId());
        }
    }


    private void createEnhanceFinishedNotification(Long datasetId) {
        try {
            ...
            notification.setOperationType(NotificationOperationTypeEnum.DATASET_ENHANCE_FINISH.getCode());
    ...
    @Override
    public void taskError(Long id) {
        baseMapper.taskError(id);
    }

    @Override
    public Task getDistinctTaskByDatasetIdAndTypeDESC(Long datasetId, int type) {
        return baseMapper.getDistinctTaskByDatasetIdAndTypeDESC(datasetId, type);
    }

    @Override
    public int finishFileIfExpected(Long id, Integer expectedFinished) {
        return baseMapper.finishFileIfExpected(id, expectedFinished);
    }
}
