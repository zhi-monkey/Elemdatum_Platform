...
import { _via_file, _VIA_FILE_LOC } from './_via_file';
import { _via_view } from './_via_view';
import { _via_attribute } from './_via_attribute';
import {
  _VIA_PROJECT_ID_MARKER,
  _VIA_PROJECT_REV_ID_MARKER,
  _VIA_PROJECT_REV_TIMESTAMP_MARKER,
} from './_via_const';
import { _VIA_FLOAT_FIXED_POINT } from './_via_config';
import {
  _via_util_array_eq,
  _via_util_download_as_file,
  _via_util_download_as_json,
...
import { _via_metadata } from './_via_metadata';
...
_via_data.prototype.attribute_add = function (
  name,
  anchor_id,
  type,
  desc,
...
_via_data.prototype._cache_update_mid_list = function () {
  var vid;
  // if('mid_list'in this.cache) {
  //   this.cache.setAttribute('mid_list', {});
  // }
  this.cache.mid_list = {};
  for (var mid in this.store.metadata) {
...
// project
//
_via_data.prototype.project_save = async function (jsonPath, datasetName) {
  //jsonPath: 标注文件所需存储的相对路径，不包含接口URL等
  return await new Promise(
    async function (ok_callback, err_callback) {
      try {
        // makeAreaLabel(this.d.store.metadata);
        //生成标注信息Json
        var data_blob = new Blob([JSON.stringify(this.d.store.metadata)], {
          type: 'text/json;charset=utf-8',
        });
        await _via_util_download_as_file(data_blob, jsonPath, datasetName);
        ok_callback();
      } catch (err) {
...
_via_data.prototype.json_save = function (modelId, monitorId, customConfig) {
  //customConfig不含areaLabel的部分
  return new Promise(
    function (ok_callback, err_callback) {
      try {
        //生成标注信息Json
        const metadata = this.d.store.metadata;

        //生成areaLabel用于设备配置下发
        let areaLabel = makeAreaLabel(metadata);
        Promise.all([
          _via_util_download_as_json(modelId, monitorId, metadata, areaLabel, customConfig),
        ]).then(() => {
          ok_callback();
        });
      } catch (err) {
...
function makeAreaLabel(metadata) {
  let areaLabel = [];

  Object.values(metadata).forEach((area, index) => {
    //每个area是一个图形
    const xy = area.xy; //第一个数为类型，其余每2个数为一个点
    const tmpList = []; //临时点集

    switch (
      xy[0] //取数组第一个数
    ) {
      case 2: //正经四边形，前两个值是左上坐标，后两个值是偏移
        areaLabel.push([
          { x: xy[1], y: xy[2] }, //左上角
          { x: xy[1] + xy[3], y: xy[2] }, //右上角
          { x: xy[1] + xy[3], y: xy[2] + xy[4] }, //右下角
          { x: xy[1], y: xy[2] + xy[4] }, //左下角
        ]);
        break;
      case 8: //三个点确定的多边形
        //加入三个原始点
        tmpList.push({ x: xy[1], y: xy[2] });
        tmpList.push({ x: xy[3], y: xy[4] });
        tmpList.push({ x: xy[5], y: xy[6] });
        //计算两个极值点
        const minX = min(tmpList.map((p) => p.x));
        const minY = min(tmpList.map((p) => p.y));
        const maxX = max(tmpList.map((p) => p.x));
        const maxY = max(tmpList.map((p) => p.y));
        areaLabel.push([
          { x: minX, y: minY },
          { x: maxX, y: minY },
          { x: maxX, y: maxY },
      ...
      case 7:
        const nodeList = [];
        for (let i = 1; i < xy.length; i += 2) {
          //从第二个元素开始，每两个元素为一个点，默认已顺时针
          nodeList.push({ x: xy[i], y: xy[i + 1] });
        }
        areaLabel.push(nodeList);
        break;
      default:
        //TODO:其他未考虑的图形可在此处加入
        break;
    ...
    //小数舍入为整数
    areaLabel = areaLabel.map((area) =>
      area.map((node) => {
        return { x: Math.round(node.x), y: Math.round(node.y) };
      }),
    );
    area.areaLabel = areaLabel[index];
  });
  return areaLabel;
}
        ...
        for (var fid in via2['_via_img_metadata']) {
          ...
          for (var rid in via2['_via_img_metadata'][fid]['regions']) {
            ...
            for (var via2_aid in via2['_via_img_metadata'][fid]['regions'][rid][
              ...
              var avalue =
                via2['_via_img_metadata'][fid]['regions'][rid]['region_attributes'][via2_aid];
