 ...
 * @since 14 Jan. 2019
 */
import { _VIA_DEFAULT_ATTRIBUTE_ANCHOR_ID } from './_via_config.js';
import { _VIA_ATTRIBUTE_TYPE, _VIA_ATTRIBUTE_ANCHOR } from './_via_attribute.js';
// import {_via_util_get_svg_button} from './_via_util.js';
import {
...
_via_editor.prototype.show = function () {
  ...
  edit_metadata_label.setAttribute(
    'title',
    'Metadata corresponds to values assigned by user for the attributes.',
  );
  ...
  edit_attribute_label.setAttribute(
    'title',
    'Attribute corresponds to fields like name, color, etc. required to describe the region of interest',
  );
...
_via_editor.prototype.get_metadata_header = function () {
  ...
  for (var aid in this.d.store.attribute) {
    tr.appendChild(this.html_element('th', this.d.store.attribute[aid].aname));
  }
...
_via_editor.prototype.jump_to_metadata = function (e) {
  ...
  if (
    this.d.metadata_store[fid][mid].where_target() === _VIA_WHERE_TARGET.SEGMENT &&
    this.d.metadata_store[fid][mid].where_target() === _VIA_WHERE_SHAPE.TIME
  ) {
    this.a.preload[fid].media_annotator.media.currentTime =
      this.d.metadata_store[fid][mid].where[where_index];
  }
};
...
_via_editor.prototype.get_attribute = function (aid) {
  ...
  if (this.d.store.attribute[aid].type === _VIA_ATTRIBUTE_TYPE.TEXT) {
    ...
    option_input.setAttribute(
      'title',
      'Enter options as comma separated value with the default option prefixed using an *. For example: "a,*b,c"',
    );
    option_input.addEventListener('change', this.attribute_on_change.bind(this));
    option_input.innerHTML = _via_util_obj_to_csv(
      this.d.store.attribute[aid].options,
  ...
  if (this.d.store.attribute[aid].type === _VIA_ATTRIBUTE_TYPE.TEXT) {
    ...
    var default_value =
      this.d.store.attribute[aid].options[this.d.store.attribute[aid].default_option_id];
  ...
  switch (vartype) {
    ...
    case 'textarea':
      if (varname === 'options') {
        ...
        this.d
          .attribute_update_options_from_csv(aid, options_csv)
          .then(function (ok) {}.bind(this));
      }
      break;
    case 'select':
    case 'select-one':
...
_via_editor.prototype.metadata_del = function (e) {
  ...
  this.d.metadata_del(fid, mid).then(
    function (ok) {
...
_via_editor.prototype.on_attribute_create = function (e) {
  ...
  this.d
    .attribute_add(new_attribute_name, _VIA_DEFAULT_ATTRIBUTE_ANCHOR_ID, _VIA_ATTRIBUTE_TYPE.TEXT)
    .then(
      function (ok) {
        this.attributes_update();
        // attribute was added
