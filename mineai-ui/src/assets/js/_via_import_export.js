...
import { _VIA_RSHAPE, _VIA_METADATA_FLAG } from './_via_metadata';
...
_via_import_export.prototype.import_from_coco = function (json_str) {
  try {
    ...
    for (var i in d.categories) {
      ...
      p.attribute[aid] = new _via_attribute(
        aname,
        'FILE1_Z0_XY1',
...
_via_import_export.prototype.import_from_webvtt = function (webvtt_str, vid, subtitle_aid) {
  return new Promise(
    function (ok_callback, err_callback) {
      ...
      while (current_line_index < webvtt_lines_count) {
        ...
        if (webvtt_lines[current_line_index].includes(' --> ')) {
          if (timestamp_found) {
            ...
            var metadata = { vid: vid, z: [starttime_sec, endtime_sec], xy: [], av: {} };
      ...
      this.d.metadata_delete_all(vid, true).then(
        function (ok) {
