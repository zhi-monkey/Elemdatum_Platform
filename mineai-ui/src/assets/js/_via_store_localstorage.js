        ...
        if (existing_project_store_str !== null) {
          ...
          this._pack_store_data().then(
            function (ok) {
              ...
              this.prev_session_timestamp_str = _via_util_date_to_filename_str(
                this.prev_session_data.project_store.created,
              );
          ...
          if (this.d.data_key_list.includes(data_key)) {
            ...
            try {
              ...
              if (store_key !== this.BROWSER_ID_KEY) {
                console.log(
                  'Failed to parse data for store_key=[' +
                    store_key +
                    '], data_key=[' +
                    data_key +
                    ']',
...
_via_store_localstorage.prototype._self_test = function () {
  ...
  var fid2 = this.d.file_add(
    'testXYZ.jpg',
    _VIA_FILE_TYPE.IMAGE,
    _VIA_FILE_LOC.URIHTTP,
    'http://somerandurl.com/files/testXYZ.jpg',
  );
  ...
  this.d
    .metadata_add(fid1, [], [_VIA_SHAPE.RECT, 10, 20, 50, 100], { aid1: 'value1', aid2: 'value2' })
    .then(
      function (ok) {
        this._pack_store_data().then(
          function (ok) {
...
export { _via_store_localstorage };
