...
'use strict';
import { _VIA_SVG_NS } from './_via_const';
import { _VIA_CONFIG } from './_via_config';
import { _VIA_FILE_SELECT_TYPE } from './_via_const';
...
// load a json from static
function _via_util_load_url() {
  var text_file;
  var folder = document.getElementById('folder');
  var folderName1 = folder.options[folder.selectedIndex].value;
  var url =
    'http://localhost/dm/storage/getFileStream?fileName=mineai-data-manager/target/classes/static/labelset/' +
    folderName1 +
    '/' +
    folderName1 +
    '.json';
  ...
  axios({
    ...
    // url = res.data;
    text_file = JSON.stringify(res.data);
    console.log(text_file);
    // console.log("loading");
    _via_data.prototype._init_default_project();
    _via_data.prototype.project_load(text_file);
...
async function _via_util_download_as_file(data, jsonPath, datasetName) {
  let formData = new FormData();
  formData.append('file', data);

  await axios({
    method: 'post',
    url: MaBackendUrlEnum.DATA_MANAGER + 'storage/uploadFileNew',
    data: formData,
    params: {
      filePath: jsonPath,
...
function _via_util_download_as_json(modelId, monitorId, data, areaLabel, customConfig) {
  return maHttp
    .post(
      {
        url: 'monitorModelConfig/saveCustomConfig',
        params: {
          modelId,
          monitorId,
          areaLabel,
...
function _via_util_page_show2(page_id, action_map) {
  _via_page_action_map = action_map;
  var el = document.getElementById('via_page_container');
...
function _via_util_page_hide() {
  console.log('_via_util_page_hide');
  var el = document.getElementById('via_page_container');
