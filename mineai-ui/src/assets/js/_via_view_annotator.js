...
'use strict';
import { _VIA_DEFAULT_ZOOM_MODE, _VIA_ZOOM_SCALE_DEFAULT_INDEX } from './_via_const';
import { _VIA_RSHAPE } from './_via_metadata';
import { _via_event } from './_via_event';
import { _VIA_FILE_TYPE } from './_via_file';
import { _via_file_annotator } from './_via_file_annotator';
import { _via_util_msg_show } from './_via_util';
import { _via_temporal_segmenter } from './_via_temporal_segmenter';
...
_via_view_annotator.prototype._view_annotate_single_image = function (vid) {
  ...
  this.file_annotator[0][0] = new _via_file_annotator(
    this,
    this.d,
    this.rinput_limit,
    vid0,
    '',
    this.file_container[0][0],
  );
  // console.log("143");
  this.file_annotator[0][0]._file_load().then(
    function () {
      this.file_annotator[0][0]._rinput_enable();
    }.bind(this),
    function (err) {
      // handled by
      // _via_file_annotator._file_load_show_error_page();
      console.log(err);
    }.bind(this),
  );
};
...
_via_view_annotator.prototype._view_annotate_single_video = function (vid) {
  ...
  if (this.d.store['config']['ui'].hasOwnProperty('gtimeline_container_height')) {
    ...
    if (this.d.store['config']['ui']['gtimeline_container_height'] in via307_fix) {
      this.d.store['config']['ui']['gtimeline_visible_row_count'] =
        via307_fix[this.d.store['config']['ui']['gtimeline_container_height']];
    }
  ...
  this.file_annotator[0][0]._file_load().then(
    function (ok) {
      ...
      this.temporal_segmenter = new _via_temporal_segmenter(
        this.file_annotator[0][0],
        this.temporal_segmenter_container,
        vid0,
        this.d,
        this.file_annotator[0][0].file_html_element,
      );
      this.temporal_segmenter.on_event(
        'edit_frame_regions',
        this._ID,
        this.file_annotator[0][0]._on_event_edit_frame_regions.bind(this.file_annotator[0][0]),
      );
    }.bind(this),
    function (err) {
...
_via_view_annotator.prototype._view_annotate_two_images = function (vid) {
  ...
  this.file_annotator[0][1] = new _via_file_annotator(
    this,
    this.d,
    this.rinput_limit,
    vid1,
    'Image 2',
    this.file_container[0][1],
  );
...
// view container
//
_via_view_annotator.prototype._view_split_content_container = function (
  container,
  file_container,
  nrow,
  ncol,
  ...
  if (_VIA_RSHAPE.hasOwnProperty(shape)) {
    ...
    switch (this.region_draw_shape) {
      case _VIA_RSHAPE.POINT:
        _via_util_msg_show('【不支持配置下发】单击以定义特征点。');
        break;
      case _VIA_RSHAPE.RECTANGLE:
        _via_util_msg_show('【支持配置下发】单击并拖动鼠标绘制矩形区域');
        break;
      case _VIA_RSHAPE.EXTREME_RECTANGLE:
        _via_util_msg_show(
          '【支持配置下发】单击并定义矩形对象的左、上、右和下（可以按任意顺序）端点，以定义其边界框。',
        );
        break;
      case _VIA_RSHAPE.CIRCLE:
        _via_util_msg_show('【不支持配置下发】单击并拖动鼠标光标以定义圆形区域');
        break;
      case _VIA_RSHAPE.EXTREME_CIRCLE:
        _via_util_msg_show('【不支持配置下发】单击并定义圆周上的三个点以定义圆形区域。');
        break;
      case _VIA_RSHAPE.ELLIPSE:
        _via_util_msg_show('【不支持配置下发】单击并拖动鼠标光标以定义椭圆区域。');
        break;
      case _VIA_RSHAPE.LINE:
        _via_util_msg_show('【支持配置下发】单击并拖动鼠标绘制直线。');
        break;
      case _VIA_RSHAPE.POLYLINE:
        _via_util_msg_show('【不支持配置下发】单击以定义多段线的顶点，并在最后一个顶点完成单击。');
        break;
      case _VIA_RSHAPE.POLYGON:
        _via_util_msg_show(
          '【支持配置下发】单击确定多边形的顶点，按下<span class="key">Enter</span>完成绘制多边形。',
        );
        break;
    }
  }
...
_via_view_annotator.prototype._metadata_on_update = function (e) {
  ...
  if (mid) {
    ...
    this.d.metadata_update_av(this.vid, mid, aid, avalue).then(
      function (ok) {
    ...
    var av = {};
    av[aid] = avalue;
    this.d.metadata_add(this.vid, [], [], av).then(
      function (ok) {
...
_via_view_annotator.prototype._attribute_html_element = function (aid, onchange_handler, mid) {
  ...
  switch (this.d.store.attribute[aid].type) {
    ...
    case _VIA_ATTRIBUTE_TYPE.SELECT:
      ...
      var selected_oid = '';
      if (mid !== '' && this.d.store.metadata[mid].av.hasOwnProperty(aid)) {
...
_via_view_annotator.prototype._on_event_keydown = function (e) {
  ...
  if (this.view_mode === _VIA_VIEW_MODE.IMAGE1 || this.view_mode === _VIA_VIEW_MODE.IMAGE2) {
...
_via_view_annotator.prototype._update_via_rinput_limit = function (n) {
  this.rinput_limit = n;
};
export { _via_view_annotator, _VIA_VIEW_MODE, _VIA_PAGE };
