...
import { _via_util_file_select_local, _via_util_page_show } from './_via_util';
import { _VIA_FILE_SELECT_TYPE } from './_via_const.js';
import { _via_util_infer_file_type_from_filename } from './_via_util';
import { _VIA_FILE_LOC } from './_via_file';
import { _via_util_msg_show } from './_via_util';
import { _VIA_FILE_TYPE } from './_via_file';
import { _via_util_file_type_to_str } from './_via_util';
...
_via_view_manager.prototype._file_add_from_filelist = function (filelist) {
  this.d.view_bulk_add_from_filelist(filelist).then(
    function (ok) {
      ...
      _via_util_msg_show(
        'Added ' + ok.fid_list.length + ' files. ' + JSON.stringify(filetype_summary),
      );
    }.bind(this),
    function (err) {
...
_via_view_manager.prototype._on_add_media_local = function () {
  _via_util_file_select_local(
    _VIA_FILE_SELECT_TYPE.IMAGE | _VIA_FILE_SELECT_TYPE.VIDEO | _VIA_FILE_SELECT_TYPE.AUDIO,
    this._file_add_local.bind(this),
    true,
  );
};

// _via_view_manager.prototype._load_dataset_list = function() {
//   _via_util_page_show('page_dataset_choose');
// }

_via_view_manager.prototype._load_minio_bucket = function (url) {
  ...
  axios({
    ...
    // url = res.data;
    let via_page_fileuri_urilist = document.getElementById('via_page_fileuri_urilist');
    console.log(via_page_fileuri_urilist);
    via_page_fileuri_urilist.whiteSpace = 'pre-line';
    via_page_fileuri_urilist.innerHTML = res.data;
  });
};
...
_via_view_manager.prototype._on_add_media_bulk = function () {
  _via_util_file_select_local(
    ...
    this._on_add_media_bulk_file_selected.bind(this),
    false,
  );
};
