...
export const putObject = (uploadUrl, file, options = {}) => {
  const { objectName, errCallback } = options;
  // 加载进度
...
export const minIOUpload = async (
  ...
  // 文件名进行混淆
  fileList.forEach((d) => {
    d.name = renameFile(d.name);
  });

  const mapper = async (d) => {
    const uploadPrefix = `${minIOPrefix}/${bucketName}/${CZ_FOLDER}`;
    ...
    if (typeof transformFile === 'function') {
      return await transformFile(fileRes, d);
    }
    return fileRes;
  };

  return await pMap(fileList, mapper, { concurrency: 10 });
};
...
export const renameFile = (name) => {
  const extname = getFileExtension(name);
  const basename = getBasename(name, extname);
  // 使用正则表达式匹配文件名是否包含 userId 和时间戳，如果包含则直接返回。因为不知道为啥这够吧方法会被执行两次(F**K)。
  const regex = new RegExp(`-${currentUserId}-\\d+${extname}$`);
  if (regex.test(name)) {
    return name;
  }
  return `${basename}-${currentUserId}-${Date.now()}${extname}`;
};
