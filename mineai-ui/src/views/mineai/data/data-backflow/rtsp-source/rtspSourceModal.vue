...
<script setup lang="ts">
  ...
  import { createRtspSource, getRtspPublicKey, updateRtspSource } from './api';
  ...
  const getTitle = computed(() => (!unref(isUpdate) ? '新增摄像头视频流' : '编辑摄像头视频流'));
  ...
  async function handleSubmit() {
    try {
      ...
      if (unref(isUpdate) && values.id) {
        await updateRtspSource(values.id, values);
      } else {
        await createRtspSource(values);
      }
