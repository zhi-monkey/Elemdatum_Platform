<template>
  <div id="dataset-detail">
    ...
    >
      ...
      <template #toolbar>
        <Button type="primary" @click="handleUpload">上传图片</Button>
        <Button type="primary" @click="batchDownload" :disabled="canDelete">批量下载</Button>
        <Button type="primary" @click="handleMultipleDelete" :disabled="canDelete">
          批量删除
        </Button>
      </template>
...
<script setup lang="ts">
  ...
  import { columns, searchFormSchema } from '/@/views/mineai/data/data-repo/details/image-file';
  import {
    del,
    queryFile,
    batchQueryFile,
  ...
  import { onMounted, ref, Ref, createVNode, onActivated, computed } from 'vue';
  ...
  import UploadDataFile from '/src/views/mineai/data/data-repo/details/upload-datafile-inline.vue';
  ...
  async function getTableData() {
    // let response = await getDataRepoFiles({ datasetId: props.id, fileType: 0 });
  ...
  //   console.log(urls);
  //   withLabel
  //     ? openModal(true, { batchDownload: true, ids: ids, urls: urls, withLabel: withLabel })
