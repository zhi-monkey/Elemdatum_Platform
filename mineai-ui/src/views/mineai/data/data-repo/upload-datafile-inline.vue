  ...
  >
    ...
    <template #header>
      <el-tabs v-model="activeTabName" class="demo-tabs" @tab-click="handleTabClick">
        ...
        <el-tab-pane label="上传其他" name="other" :disabled="otherTabStat" />
      </el-tabs>
    </template>
    <template #footer>
      <div v-show="state.uploadStep === 0">
...
<script setup lang="ts">
  ...
  import UploadInline from './details/components/UploadForm/inline.vue';
  import {
    getFileFromMinIO,
    withDimensionFile,
    trackUploadProps,
  ...
  const handleTabClick = (pane) => {
    state.row.dataType = getTypeMap(pane.paneName);
    const nextParams = buildDataParams(state.row.dataType);
    Object.assign(state, nextParams);
  };

  // 上传文件接口
  const submit = async (datasetId, files) => {
    // 构造请求体数组
    const dataRepoFileCreateDTOs = files.map((file) => {
      // 从URL中截取文件名
    ...
    // 更新请求URL和数据结构以匹配新的批量创建接口
    return await maHttp.post(
      {
        url: `/datarepofiles/batch-create`, // 更新为新的批量创建端点
        headers: {},
        data: dataRepoFileCreateDTOs, // 直接传递DTOs数组作为请求体
      },
  ...
  onMounted(() => {
    Object.assign(state, buildDataParams(getTypeMap(activeTabName.value)));
  });
  ...
  const uploader = async (datasetId, files) => {
    // 截取文件名并判断文件类型
    files.forEach((file) => {
      const fileName = file.url.split('/').pop(); // 截取URL最后一部分作为文件名
      file.type = getFileType(fileName); // 调用getFileType函数获取文件类型并添加到对象中
    });
  ...
  const optionCreateProps = computed(() => {
    ...
    const props = {
      params: {
        ...
        // dataset/8/origin
        objectPath: `datarepo/${state.row.id}/${state.isVideo ? 'video' : 'origin'}`, // 图片/视频对象存储路径
      },
    };
  ...
  function getTypeMap(typename) {
    // 判断文件类型
    if (typename == 'image') {
      return dataTypeCodeMap.IMAGE;
    } else if (typename == 'video') {
      return dataTypeCodeMap.VIDEO;
    } else {
      return dataTypeCodeMap.OTHER;
    }
  }
  ...
  watch(
    () => activeTabName.value,
    (next) => {
      const nextParams = buildDataParams(getTypeMap(activeTabName.value));
      Object.assign(state, nextParams);
    },
  );
