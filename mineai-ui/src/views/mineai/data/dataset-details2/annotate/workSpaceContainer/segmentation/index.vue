...
<script>
  ...
  import { last, isFunction, isEmpty } from 'lodash-es';
  import { raise, replace, getPolygonExtent } from '/@/utils/dubhe';
  import { useDraw } from '/@/hooks/dubhe';
  ...
  export default {
    ...
    setup(props, ctx) {
      ...
      // 上传标注校验格式校验
      const validate = () => {
        return isEmpty(state.unfinishedShape);
      };
      ...
      const syncShapes =
        ...
        (nextState) => {
          ...
          // state: 数据
          // options: event 等事件参数
          fileChange({ type, state: nextState, options });
        };
      ...
      // 每次拖拽的优先级提升
      const onDragStart = (draw, shape, event) => {
        ...
        // 同步当前标注
        props.setCurAnnotation(shape);
        const options = {
          shape: shape.data.categoryId,
          event,
        };
        fileChange({ type: 'DRAG_START', state, options });
      };
      ...
      const onDragMove = ({ drag }, shape) => {
        ...
        fileChange({ type: 'DRAG_MOVE', state });
      };
      ...
      const onDragEnd = ({ drag }, shape) => {
        props.updateState(
          ...
          (nextState) => fileChange({ type: 'DRAG_END', state: nextState }),
        );
      };
