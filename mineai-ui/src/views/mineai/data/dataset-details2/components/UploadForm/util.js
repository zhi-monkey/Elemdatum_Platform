...
export const putObject = (uploadUrl, file, options = {}) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.upload.addEventListener('progress', function (e) {
      if (e.lengthComputable && typeof callback === 'function') {
...
export const minIOUpload = async (
  ...
  const mapper = async (d) => {
    ...
    if (typeof callback === 'function' && fileList.length >= 1) {
      // 计算整体进度
      const overallProgress = (resolved / fileList.length) * 100;
      callback(overallProgress, fileList);
    }
...
export const newMinIOUpload = async (id, objectPath, fileList, autoInsertDatabase = true) => {
  ...
  const mapper = async (d) => {
    ...
    // 保存上传进度（根据参数决定是否自动存库）
    if (autoInsertDatabase) {
      await uploadStore.setUploadState(id, transformed);
    } else {
      await uploadStore.setUploadStateWithoutInsertDataBase(id, transformed);
    }

    // 更新上传计数并触发进度更新
    uploadedCount++;
    const progress = (uploadedCount / fileList.length) * 100;
    // 这里可以通过某种方式通知进度更新，但考虑到当前架构，我们依赖store状态更新
