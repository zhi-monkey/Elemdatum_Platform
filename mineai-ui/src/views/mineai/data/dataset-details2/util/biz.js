...
// 解析annotation信息 标注详情处使用，id字段改为名称
export const parseAnnotationInfo = (annotationStr, labels) => {
  let result = [];
  let isBbox = false;
  try {
    const annotation = JSON.parse(annotationStr);
    result = annotation.map((d) => {
      const data = {
        score: d.score,
        bbox: parseBbox(d.bbox),
        name: (labels.find((label) => label.id === d.category_id) || {}).name || '',
        color: (labels.find((label) => label.id === d.category_id) || {}).color || '#000',
      };
      if (d.segmentation) {
        data.points = d.segmentation.map(rawArr2Pos);
...
export const buildUrlItem = (d) => ({
  url: `${bucketName}/${decodeURIComponent(d.data.objectName)}`,
...
const defaultTransform = (d) => ({
  id: d.id, // 提取文件ID
  name: d.name, // 文件名
  status: d.status, // 文件状态
  datasetId: d.datasetId, // 数据集ID
  url: `${bucketHost}/${d.url}`, // 拼接URL
  updateUserId: d.updateUserId, // 更新用户ID
  annotation: d.annotation, // 标注信息
  pid: d.pid, // 父ID
  enhanceType: d.enhanceType, // 增强类型
  prediction: d.prediction, // 预测信息
  labelId: d.labelId, // 标签ID
  abstractName: d.abstractName, // 抽象名称
  abstractUrl: d.abstractUrl, // 抽象URL
  deleted: d.deleted, // 删除标志
  createUserId: d.createUserId, // 创建用户ID
  createTime: d.createTime, // 创建时间
  updateTime: d.updateTime, // 更新时间
  fileType: d.fileType, // 文件类型
  width: d.width, // 文件宽度
  height: d.height, // 文件高度
  originUserId: d.originUserId, // 原始用户ID
  esTransport: d.esTransport, // ES传输标志
  excludeHeader: d.excludeHeader, // 是否排除头部
});
...
export const dataTypeCodeMap = {
  ...
  AUDIO: 4,
  POINT_CLOUD: 5, // 点云
  // 6: 独立视频（沿用现有 6 码，见 datasetCode/util）
  CUSTOM: 100,
};
...
export const datasetCode = [
  {
    id: 0,
    text: '导入自定义数据集',
    code: 'ts-cli dataset import --type=custom --source /Users/myDataset --annotation_type=custom --name=自定义数据集',
  },
  {
    id: 1,
    text: '导入标准数据集',
    code: 'ts-cli dataset import --type=standard --source /Users/myDataset --annotation_type=ImageClassify --name=图像分类数据集',
  },
];
