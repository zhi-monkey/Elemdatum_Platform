...
<script setup lang="ts">
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (data?.record) {
      ...
      taskType.value = data.record.taskType;

      // 根据任务类型设置对应的字段
      const formData: any = {
        taskName: data.record.taskName,
        taskType: data.record.taskType,
        datasetId: getMockDatasetIdByName(data.record.datasetName),
        params: '{"batch_size": 32, "augmentation": true}',
        description: '这是一个数据处理任务',
      };

      if (data.record.taskType === 'template') {
        formData.templateId = getMockTemplateIdByName(data.record.operatorName);
      } else if (data.record.taskType === 'builtin') {
        formData.builtinOperatorId = getMockBuiltinIdByName(data.record.operatorName);
      } else if (data.record.taskType === 'custom') {
        formData.customOperatorId = getMockCustomIdByName(data.record.operatorName);
      }
  ...
  // 辅助函数：根据名称获取对应的ID（模拟）
  function getMockDatasetIdByName(name: string) {
    const dataset = mockDatasets.find((d) => d.label === name);
    return dataset?.value || 1;
  }

  function getMockTemplateIdByName(name: string) {
    const template = mockTemplates.find((t) => t.label === name);
    return template?.value || 1;
  }

  function getMockBuiltinIdByName(name: string) {
    const operator = mockBuiltinOperators.find((o) => o.label === name);
    return operator?.value || 1;
  }

  function getMockCustomIdByName(name: string) {
    const operator = mockCustomOperators.find((o) => o.label === name);
    return operator?.value || 1;
  }
  ...
  async function handleSubmit() {
    try {
      ...
      // 模拟提交
      console.log('编辑提交数据:', { ...recordData.value, ...values });
