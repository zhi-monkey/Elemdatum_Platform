<template>
  <BasicModal v-bind="$attrs" title="转换镜像上传" @register="registerModal">
    <card>
      ...
      <div class="p2">
        ...
        <radio-group button-style="solid" v-model:value="type">
          <radio-button value="url上传" @click="functionSelect1"
            >镜像仓库地址</radio-button
          >
          <radio-button value="本地上传" @click="functionSelect2"
            >本地文件上传</radio-button
          >
        </radio-group>
        <div>
...
<script lang="ts" setup>
  ...
  const link = ref('');
  const uploadLinkURL = ref('model/uploadUrlImage');
  const uploadLocal = ref('model/uploadLocalModel');
