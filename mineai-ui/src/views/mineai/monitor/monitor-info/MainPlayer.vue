<template>
  ...
  >
    <div style="position: absolute; top: 5%; right: 5%" v-if="showBtn">
      <Button
        type="primary"
        ghost
        size="small"
...
<script lang="ts" setup>
  ...
  const createJessibuca = () => {
    ...
    _jessibuca.on('error', async function (error: any) {
      ...
      if (_jessibuca !== null) {
        ...
        _jessibuca = null;
        await playVideo(videoUrl.value);
      }
    });
  };
      ...
      _jessibuca.on('play', () => {
        if (videoInfo.value !== null) {
          ...
          _jessibuca.updateWatermark(
            !showModelStream.value && props.contentList !== undefined
              ? [
                  {
                    //opacity: 0.5, // 透明度
                    ...
                    // 文字配置
                    text: {
                      content: showModelStream.value ? '' : props.title,
                      color: 'white',
  ...
  watch(videoUrl, async (newVal) => {
    if (_jessibuca !== null) {
      await _jessibuca.destroy();
      _jessibuca = null;
    }
    await playVideo(newVal);
  });
