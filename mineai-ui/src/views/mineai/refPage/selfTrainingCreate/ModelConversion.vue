    ...
    <a-row :gutter="[8, 16]" style="height: 100%">
      <a-col :span="12" style="height: 87vh">
        <div style="height: 95%" class="flex flex-col">
          <a-card title="转换详情" style="height: 60vh" class="myCard">
            <template #extra>
              <a-button type="primary" style="margin-right: 10px; background-color: #2e62bf">
                开始转换
              </a-button>
              <a-button type="primary" style="background-color: #2e62bf" @click="goToNextStep">
                下一步
              </a-button>
            </template>
          ...
          <a-card title="转换进度" style="height: 15vh; padding-bottom: 4px" class="myCard">
...
<script setup lang="ts">
  import { defineEmits, defineProps, onMounted, onUnmounted, Ref, ref } from 'vue';
  import {
    Button as AButton,
    Card as ACard,
    Col as ACol,
  ...
  import { MaxAccList, newAccSeries, newLossSeries, schema } from './detailsData';
  ...
  import { fetchConvertDataMock } from '/@/views/mineai/refPage/generationGuide/api/ConvertApi';
  ...
  async function getData(jobId) {
    return await fetchConvertDataMock();
  }

  // 获取作业参数
  async function getParams(jobId: number) {
  ...
  const emit = defineEmits(['next']); // 定义发射事件

  // 定义下一步的处理方法
  const goToNextStep = () => {
    emit('next'); // 触发父组件的下一步事件
  };
</script>
