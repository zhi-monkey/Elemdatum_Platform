<template>
  <div id="cards">
    <div class="card-item" v-for="(card, i) in cards" :key="card.title">
      ...
      <charts class="ring-charts" :option="card.ring" />
      <div class="card-footer">
        <div class="card-footer-item">
          <div class="footer-title">累计金额</div>
          <div class="footer-detail">
            <digital-flop :config="card.total" style="width: 70%; height: 35px" />
            元
          </div>
        </div>
        <div class="card-footer-item">
          ...
          <div class="footer-detail">
            <digital-flop :config="card.num" style="width: 70%; height: 35px" />
            处
          </div>
        </div>
      </div>
...
<script lang="ts" setup>
  ...
  import { Charts, DigitalFlop } from '@kjgl77/datav-vue3';

  let cards: Ref<any[]> = ref([]);
  ...
  function createData() {
    cards.value = new Array(5).fill(0).map((foo, i) => ({
      title: '测试路段' + (i + i),
      total: {
