<template>
  <div id="digital-flop">
    ...
    <div class="digital-flop-item">
      ...
      <div class="digital-flop-content" style="width: 180px; height: 100px">
        <div class="digital-flop-number" style="color: #4d99fc">{{ controllerAllNum }}</div>
      </div>
    </div>
...
<script lang="ts" setup>
  ...
  const getClusterAllInfo = async () => {
    let clusterAllInfo;
    clusterAllInfo = await maHttp.get(
      {
        url: 'controller/getClusterAllInfo',
        headers: {
          // @ts-ignore
          ignoreCancelToken: true,
        },
    ...
    //获取cpu，内存，硬盘利用率，并赋值
    cpuUsed_show.value = parseFloat(
      (parseFloat(clusterAllInfo.cluster_cpu_utilisation) * 100).toFixed(2),
    );
    cpuUsedConfig.data = [cpuUsed_show.value];
    memoryUsed_show.value = parseFloat(
      (parseFloat(clusterAllInfo.cluster_memory_utilisation) * 100).toFixed(2),
    );
    memoryUsedConfig.data = [memoryUsed_show.value];
    hardDiskUsed_show.value = parseFloat(
      (parseFloat(clusterAllInfo.cluster_disk_size_utilisation) * 100).toFixed(2),
    );
    hardDiskUsedConfig.value = hardDiskUsed_show.value;
    hardDiskTotalSize.value = Math.round(
      parseInt(clusterAllInfo.cluster_disk_size_capacity, 10) / (1024 * 1024 * 1024),
    );

    hardDiskUnusedSize.value = Math.round(
      parseInt(
        clusterAllInfo.cluster_disk_size_capacity - clusterAllInfo.cluster_disk_size_usage,
        10,
      ) /
        (1024 * 1024 * 1024),
    );
    controllerAllNum.value = parseInt(clusterAllInfo.cluster_node_online);
  };
  ...
  onMounted(async () => {
    await Promise.all([getControllerNum(), getHardwareNum(), getLoadNum(), getClusterAllInfo()]);
    await createData();
