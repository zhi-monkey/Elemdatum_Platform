...
<script lang="ts" setup>
  ...
  const emits = defineEmits(['updateDeploy']);
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    DeployAdd.value = data.DeployAdd;
    isUpdate.value = data.isUpdate;
    if (DeployAdd.value === false) {
      isUpdate.value = true;
      description.value = data.deployDescription;
  ...
  const getTitle = computed(() => (!unref(isUpdate) ? '推理镜像上传' : '推理镜像编辑'));
