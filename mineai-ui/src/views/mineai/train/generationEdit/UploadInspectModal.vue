...
<script lang="ts" setup>
  ...
  const emits = defineEmits(['updateInspect']);
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    InspectAdd.value = data.InspectAdd;
    isUpdate.value = data.isUpdate;
    if (InspectAdd.value === false) {
      isUpdate.value = true;
      description.value = data.testDescription;
  ...
  const getTitle = computed(() => (!unref(isUpdate) ? '质检镜像上传' : '质检镜像编辑'));
