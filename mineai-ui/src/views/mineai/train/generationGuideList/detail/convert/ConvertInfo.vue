...
<script lang="ts" setup>
  ...
  // 计算属性
  const isConverting = computed(() => data.value.status === 1);
  const isSuccess = computed(() => data.value.status === 6);
  ...
  onMounted(async () => {
    if (intervalBus.canSet.value && !intervalBus.isRefreshIntervalCExists()) {
