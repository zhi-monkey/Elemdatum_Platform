...
export const columns: BasicColumn[] = [
  ...
  },
  {
    ...
    customRender: ({ record }) => {
      let text;
      let color;
      if (record.annotationType === 'Detection') {
        text = '目标检测';
        color = 'yellow';
      } else if (record.annotationType === 'Segmentation') {
        text = '目标分割';
        color = 'blue';
      }
      return h(Tag, { color: color }, () => text);
  ...
  {
    title: '标注格式',
    dataIndex: 'annotationFormat',
    width: 60,
    customRender: ({ record }) => {
      let text;
      let color;
      if (record.annotationFormat === 'YOLO') {
        text = 'YOLO';
        color = 'yellow';
      } else if (record.annotationFormat === 'COCO') {
        text = 'COCO';
        color = 'blue';
      }
      return h(Tag, { color: color }, () => text);
  ...
  },
  {
    ...
    width: 60,
    customRender: ({ record }) => {
      const currentText = record.chipType;
      const currentContent = record.chipType;
      return h(
        Popover,
        {
          content: currentContent,
...
/!*  {
  ...
  },*!/
  {
    ...
    component: 'ApiSelect',
    componentProps: {
      api: findAllChips, // 调用 findAllChips API
      labelField: 'chipType', // 下拉框中显示的数据字段
      valueField: 'id',   // 选中值对应的字段
      immediate: true,    // 立即加载数据
      resultField: 'data', // 返回数据字段
    },
  },
...
function setCurrentItem(e): any {
  const fieldNameList = ['showName', 'description', 'chipType', 'vagueInfo'];
  ...
  for (const item of fieldNameList) {
    if (e === item) {
      result.push({ field: item, ifShow: true });
    } else {
      result.push({ field: item, ifShow: false });
    }
  }
  return result;
}
