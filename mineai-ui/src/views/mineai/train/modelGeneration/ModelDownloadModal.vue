...
<script lang="ts" setup>
  ...
  const [registerModal, { closeModal, setModalProps }] = useModalInner(async (data) => {
    ...
    await handleDownloadWeightFileListByJobId(data.jobId);
  ...
  async function handleSubmit() {
    ...
    if (selectedFiles.length === 0) {
      createMessage.warning('请先选择要下载的模型！');
      return;
    }
  ...
  async function handleDownloadWeightFileListByJobId(jobId) {
    try {
      const response = await maHttp.get<string[]>(
        {
          url: 'model/getWeightPathByJobId',
          params: { jobId: jobId },
      ...
      if (!response || response.length === 0) {
        createMessage.warning('暂无可下载的权重文件（可能任务已取消或文件已被清理）。');
        return;
      }
