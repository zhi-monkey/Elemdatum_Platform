...
<script setup lang="ts">
  ...
  const confidence = ref<number>(0.35); // 置信度值，默认0.35
  ...
  const startTest = async () => {
    // 检查一下置信度值，保险起见
    if (confidence.value === null || confidence.value < 0 || confidence.value > 100) {
      ...
      await maHttp.post(
        {
          ...
          params: {
            ...
            params: {
              MODE_WORKING_MODE: modelInfo.params.MODE_WORKING_MODE,
              HP_CONFIDENCE: confidence.value.toFixed(2),
            },
          },
          headers: {
            // @ts-ignore
