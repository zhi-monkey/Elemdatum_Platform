...
<script lang="ts" setup>
  ...
  const ACTIVE_STATUSES = [0, 1, 11, 12];
  const TRAIN_SUCCEEDED_STATUS = 2;
  ...
  async function pollDashboardData() {
    ...
    try {
      ...
      const isJobTerminated = !ACTIVE_STATUSES.includes(jobData.status);
      if (isJobTerminated) {
        ...
        clearInterval(pollingTimer);
      } else if (realPercent.value >= 100) {
        // 自迭代场景允许按进度 100% 直接判定前端训练成功，避免用户停留在“训练中”
        if (!trainingComplete.value) {
          trainingComplete.value = true;
          realPercent.value = 100;
          data.value = {
            ...jobData,
            status: TRAIN_SUCCEEDED_STATUS,
          };
