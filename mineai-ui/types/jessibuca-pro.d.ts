declare namespace JessibucaPro {
  ...
  interface Config {
    ...
    useMSE?: boolean;
    /**
     * 是否使用ZlmediaKit的WebRTC模式
     */
    isWebrtcForZLM?: boolean;
    /**
     * 是否开启Webcodecs硬解码
...
declare class JessibucaPro {
   ...
   */
  on(
    event: JessibucaPro.EVENTS.videoInfo,
    callback: (data: {
      /** 视频宽 */
      width: number;
      /** 视频高 */
   ...
   */
  on(
    event: JessibucaPro.EVENTS.audioInfo,
    callback: (data: {
      /** 声频通道 */
   ...
   */
  on(
    event: JessibucaPro.EVENTS.timeout,
    callback: (error: JessibucaPro.EVENTS.timeout) => void,
  ): void;
   ...
   */
  on(
    event: JessibucaPro.EVENTS.stats,
    callback: (stats: {
      /** 当前缓冲区时长，单位毫秒 */
      buf: number;
      /** 当前视频帧率 */
   ...
   */
  on(
    event: JessibucaPro.EVENTS.performance,
    callback: (times: {
   ...
   */
  on(
    event: JessibucaPro.EVENTS.playbackSeek,
    callback: (times: { hour: number; min: number; second: number }) => void,
  ): void;
   ...
   */
  on(
    event: JessibucaPro.EVENTS.playbackStats,
    callback: (stats: {
      fps: number;
      start: number;
      end: number;
   ...
   */
  on(
    event: JessibucaPro.EVENTS.playbackTimestamp,
    callback: (times: { hour: number; min: number; second: number; ts: number }) => void,
  ): void;
   ...
   */
  on(
    event: JessibucaPro.EVENTS.crashLog,
    callback: (log: {
      url: string;
      error: string;
      playType: string;
